...
# 套件: 目录隔离 -- 永不冲突的并行执行通道
"""
s12_worktree_task_isolation-zh.py - 工作树 + 任务隔离
...
def detect_repo_root(cwd: Path) -> Path | None:
    """如果 cwd 在 git 仓库内则返回仓库根目录，否则返回 None。"""
    try:
        r = subprocess.run(
...
class TaskManager:
    ...
    def list_all(self) -> str:
        ...
        for t in tasks:
            ...
            wt = f" 工作树={t['worktree']}" if t.get("worktree") else ""
...
# -- WorktreeManager: 创建/列表/运行/移除 git 工作树 + 生命周期索引 --
class WorktreeManager:
    ...
    def _run_git(self, args: list[str]) -> str:
        ...
        if not self.git_available:
            raise RuntimeError("不在 git 仓库中。工作树工具需要 git。")
        r = subprocess.run(
            ["git", *args],
            cwd=self.repo_root,
            capture_output=True,
    ...
    def _validate_name(self, name: str):
        ...
        if not re.fullmatch(r"[A-Za-z0-9._-]{1,40}", name or ""):
            raise ValueError(
                "无效的工作树名称。请使用 1-40 个字符: 字母、数字、.、_、-"
            )

    def create(self, name: str, task_id: int = None, base_ref: str = "HEAD") -> str:
        """创建一个 git 工作树，可选绑定到任务"""
        self._validate_name(name)
        if self._find(name):
            raise ValueError(f"工作树 '{name}' 在索引中已存在")
...
TOOLS = [
    ...
    {
        "name": "task_list",
        "description": "列出所有任务，包含状态、负责人和工作树绑定信息。",
    ...
    },
    {
        ...
        "description": "创建一个 git 工作树，可选绑定到任务。",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string"},
    ...
    {
        "name": "worktree_run",
        "description": "在指定工作树目录中运行 shell 命令。",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string"},
    ...
    },
    {
        ...
        "description": "移除工作树，可选将其绑定的任务标记为已完成。",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string"},
    ...
    {
        "name": "worktree_keep",
        "description": "将工作树标记为已保留状态，不实际移除。",
        "input_schema": {
            "type": "object",
            "properties": {"name": {"type": "string"}},
            "required": ["name"],
